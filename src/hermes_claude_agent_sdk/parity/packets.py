...
FORBIDDEN = frozenset(("raw", "content", "prompt", "transcript", "session_token", "auth", "credential", "secret", "provider_payload", "exception", "environment", "filesystem_path", "customer", "customer_data", "tool_args", "tool_results", "arguments", "headers", "cookies", "fixture_bytes"))
...
def _obj(value: Any, keys: set[str] | frozenset[str], label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping) or set(value) != set(keys):
        _fail(f"{label} must have exactly {sorted(keys)}")
    if any(key in FORBIDDEN for key in value):
        _fail(f"{label} contains forbidden content")
    return dict(value)
...
def _sha1(value: Any, label: str) -> str:
    if not isinstance(value, str) or len(value) != 40 or any(char not in "0123456789abcdef" for char in value):
        _fail(f"{label} must be lowercase SHA-1")
    return value

def _version(value: Any, label: str) -> str:
    value = _id(value, label)
    if len(value.split(".")) != 3 or any(not part.isdigit() for part in value.split(".")): _fail(f"{label} must be semver")
    return value
...
def _terminal(value: Any, label: str) -> dict[str, Any]:
    ...
    _enum(result["kind"], TERMINAL, f"{label}.kind")
    _int(result["count"], f"{label}.count", 0, 1)
    return result

def derive_path_qualification(expected: str, status: str) -> str:
    _enum(expected, EXPECTED, "expected_outcome")
    _enum(status, OBSERVED, "path status")
    if status == "ENVIRONMENT_BLOCKED":
        return "BLOCKED"
    if status == "PENDING":
        return "PENDING"
    if status == "NOT_APPLICABLE":
        return "NOT_APPLICABLE" if expected == status else "FAIL"
...
def validate_redaction(value: Any) -> dict[str, Any]:
    ...
    if result["profile"] != "v3-safe":
        _fail("redaction profile is invalid")
    ...
    if any(not isinstance(item, str) or item not in REDACTION_CATEGORIES for item in categories):
        _fail("redaction category is not in the closed enum")
...
def _trace_codes(value: Any, label: str) -> list[str]:
    ...
    if any(not isinstance(code, str) or code not in TRACE_REGISTRY for code in codes):
        _fail(f"{label} contains an unknown trace code")
    return codes
...
def validate_path_status(value: Any, expected: str, label: str = "path_status") -> dict[str, Any]:
    ...
    if terminal["count"] != expected_count or terminal["kind"] != expected_kind and not (status in {"EXPECTED_NEGATIVE", "VERIFIED_FAILURE"} and terminal["kind"] == "cancelled"):
        _fail(f"{label}.terminal does not match status")
    terminals = [code for code in traces if code.startswith("terminal.")]
    if status in {"ENVIRONMENT_BLOCKED", "PENDING", "NOT_APPLICABLE"} and traces:
        _fail(f"{label} non-executed status has a trace")
    if status in {"PASS", "EXPECTED_NEGATIVE", "VERIFIED_FAILURE"} and terminals != [f"terminal.{terminal['kind']}"]:
        _fail(f"{label} must contain its one terminal trace")
    qualification = derive_path_qualification(expected, status)
    if result["qualification"] != qualification:
        _fail(f"{label}.qualification is not derived")
    return result
...
def _state(value: Any, label: str) -> dict[str, Any]:
    ...
    for key, allowed in (("lifecycle", ("fresh", "bound", "running", "completed", "failed", "cancelled", "closed")), ("approval", ("not_required", "pending", "granted", "denied", "late_rejected")), ("tool", ("none", "requested", "executed", "denied", "cancelled", "failed", "recovered")), ("resume", ("absent", "supplied", "accepted", "rejected")), ("billing", ("included", "blocked", "unknown", "not_applicable"))):
        _enum(result[key], allowed, f"{label}.{key}")
...
def _path(value: Any, label: str) -> dict[str, Any]:
    ...
    for ordinal, call in enumerate(calls, 1):
        ...
        if item["ordinal"] != ordinal:
            _fail(f"{label} tool ordinal is not contiguous")
        ...
        if mode == "none" and request["sha256"] is not None:
            _fail("request_id mode none has a digest")
    _int(result["side_effect_count"], f"{label}.side_effect_count")
    for event in _arr(result["sdk_events"], f"{label}.sdk_events", 32):
        ...
        if item["trace_code"] != SDK_EVENT_CODES[event_name]:
            _fail("SDK event mapping is invalid")
    before, after = _state(result["state_before"], f"{label}.state_before"), _state(result["state_after"], f"{label}.state_after")
    if after["side_effect_count"] - before["side_effect_count"] != result["side_effect_count"]:
        _fail(f"{label} side-effect transition is invalid")
    if not result["required"] and (expected != "NOT_APPLICABLE" or result["trace_codes"] or calls or result["sdk_events"] or result["side_effect_count"] or terminal != {"kind": "not_applicable", "count": 0}):
        _fail(f"{label} non-required declaration is not empty")
    if result["required"] and expected == "PASS" and terminal != {"kind": "complete", "count": 1}:
        _fail(f"{label} positive declaration is not complete")
    if result["required"] and expected == "EXPECTED_NEGATIVE" and terminal["kind"] not in {"failed", "cancelled"}:
        _fail(f"{label} denial declaration is not failed/cancelled")
    return result
...
def _candidate(value: Any) -> dict[str, Any]:
    ...
    if result["candidate_schema_version"] != 1 or result["sdk_distribution"] != "claude-agent-sdk" or result["sdk_version"] != "0.2.144":
        _fail("candidate envelope is invalid")
    ...
    if result["candidate_sha256"] != canonical_sha256(identity):
        _fail("candidate_sha256 does not match")
    return result
...
def _resume(value: Any, label: str = "resume") -> dict[str, Any]:
    ...
    if result["runtime_id"] != "hermes-claude-agent-sdk" or result["runtime_schema_version"] != 1:
        _fail(f"{label} runtime envelope is invalid")
    ...
    if ref == "fixture:none":
        if result["present"] or result["state_length"] or any(result[key] is not None for key in ("prior_state_sha256", "supplied_state_sha256", "produced_state_sha256")):
            _fail(f"{label} absent state has metadata")
    elif isinstance(ref, str) and ref.startswith("fixture:"):
        _id(ref[8:], f"{label}.fixture_ref")
    else:
...
def _inventory(value: Any) -> dict[str, Any]:
    ...
    if result["observed_inventory_sha256"] != canonical_sha256(_omit(result, "declared_inventory_sha256", "observed_inventory_sha256", "unknown_names", "missing_names", "schema_drift_names")):
        _fail("observed inventory hash does not match")
    return result
...
def _attempt(value: Any, expected_paths: Mapping[str, Any] | None, label: str) -> dict[str, Any]:
    ...
    trace = _arr(result["trace"], f"{label}.trace", 256)
    for seq, entry in enumerate(trace, 1):
        ...
        _sha(item["correlation_sha256"], "trace.correlation_sha256", True)
...
def _sdk_grade(value: Any) -> dict[str, Any]:
    result = _obj(value, {"ledger_sha256", "row_count", "requires_0_3_239_rows", "upgrade_issue_ref", "status"}, "grade.sdk_ledger")
    _sha(result["ledger_sha256"], "grade.sdk_ledger.ledger_sha256"); _int(result["row_count"], "grade.sdk_ledger.row_count", 0, 23)
    rows = _arr(result["requires_0_3_239_rows"], "grade.sdk_ledger.requires_0_3_239_rows", 23); [_id(row, "upgrade row") for row in rows]; _sorted(rows, "upgrade rows")
    if result["upgrade_issue_ref"] != "issue:16": _fail("SDK upgrade issue must be issue:16")
    _enum(result["status"], {"CLEAR", "STOP"}, "grade.sdk_ledger.status")
    if result["status"] != ("STOP" if rows else "CLEAR"): _fail("SDK ledger status is not derived")
    return result

def _grade(value: Any, cells: list[Mapping[str, Any]], statuses: list[str], inventory: Mapping[str, Any], billing_safe: bool, isolation_safe: bool) -> dict[str, Any]:
    result = _obj(value, GRADE_KEYS, "result.grade")
    status = "BLOCKED" if "ENVIRONMENT_BLOCKED" in statuses else "PENDING" if "PENDING" in statuses else "FAIL" if "VERIFIED_FAILURE" in statuses else "PASS"
    if not cells: status = "BLOCKED"
    if result["status"] != status: _fail("grade.status is not derived")
    ...
    for key in ("cell_qualifications", "pass_caret_3", "pass_at_3"): _derived(result[key], f"grade.{key}")
    _sdk_grade(result["sdk_ledger"])
    ...
    if result["inventory_exact"] != (not any(inventory[key] for key in ("unknown_names", "missing_names", "schema_drift_names"))) or result["billing_safe"] != billing_safe or result["isolation_safe"] != isolation_safe: _fail("grade safety fields are not derived")
    return result
...
def validate_result(value: Any, *, freeze: Mapping[str, Any] | None = None, catalog: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    cells = _arr(result["cells"], "result.cells"); ids: list[str] = []; statuses: list[str] = []; boundaries: set[str] = set(); billing_safe = True; isolation_safe = True; catalog_caps = {cap["id"]: cap for cap in catalog.get("capabilities", [])} if catalog else {}
    for index, cell in enumerate(cells, 1):
        ...
        for ordinal, attempt in enumerate(attempts, 1):
            ...
            validated = _attempt(attempt, expected_paths, f"cell[{index}].attempt[{ordinal}]")
            if validated["candidate_sha256"] != candidate["candidate_sha256"]: _fail("attempt candidate differs")
            if item["session_scope"] == "isolated_cell" and validated["boundary_sha256"] in boundaries: isolation_safe = False; _fail("isolated boundary is reused")
            boundaries.add(validated["boundary_sha256"]); resume = validated["resume"]
            if previous is not None and resume["prior_state_sha256"] != previous: _fail("resume prior state does not chain")
            previous = resume["produced_state_sha256"]; billing_safe = billing_safe and validated["billing"]["safe"]
            statuses.extend(validated["path_status"][name]["status"] for name in PATHS)
    terminal_total = sum(1 for status in statuses if status in {"PASS", "EXPECTED_NEGATIVE", "VERIFIED_FAILURE"})
    if run["terminal_event_count"] != terminal_total or run["cell_count"] != len(cells) or run["required_cell_count"] != sum(1 for cell in cells if cell["required"]): _fail("run derived counts are invalid")
    if catalog is not None and set(ids) != set(catalog_caps): _fail("result cells are not a catalog bijection")
    _grade(result["grade"], cells, statuses, inventory, billing_safe, isolation_safe); validate_redaction(result["redaction"])
...
def validate_aggregate(value: Any, *, catalog: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    packets = _arr(result["partition_packets"], "aggregate.partition_packets"); seen: set[str] = set(); all_caps: set[str] = set(); rows: list[dict[str, str]] = []
    ...
    if catalog is not None and all_caps != {cap["id"] for cap in catalog.get("capabilities", [])}: _fail("aggregate capabilities are not a catalog bijection")
    ...
    if catalog is not None and result["catalog_sha256"] != catalog.get("catalog_sha256"): _fail("aggregate catalog binding is invalid")
    return result
...
def hash_projection(field: str, value: Mapping[str, Any] | Sequence[Any]) -> str:
    ...
    if field == "source_map_sha256":
        source = _map(value, "catalog"); packs = source.get("source_packs", source); return canonical_sha256([{key: item[key] for key in item if key != "row_ids_sha256"} for item in sorted(packs, key=lambda item: item["id"])])
    if field == "rows_sha256":
        ledger = _map(value, "ledger"); return canonical_sha256(sorted([{key: row[key] for key in row if key not in {"rows_sha256", "ledger_sha256"}} for row in ledger["rows"]], key=lambda row: (row["pack_id"], row["row_id"])))
    if field == "ledger_sha256": return canonical_sha256(_omit(_map(value, "ledger"), "rows_sha256", "ledger_sha256"))
    ...
    if field in {"declared_inventory_sha256", "fixture_manifest_sha256", "scenario_sha256", "resume_sha256", "catalog_sha256", "freeze_sha256", "result_sha256"}: return canonical_sha256(_omit(_map(value, field), field))
