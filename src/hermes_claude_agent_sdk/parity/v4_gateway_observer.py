...
_CHILD = ("child", "subagent", "delegat", "background")
...
def _digest(value: object) -> tuple[int, str]:
    try:
        ...
        _fail("observed frame is not bounded JSON")
    if len(raw) > 1_048_576:
        _fail("observed frame exceeds the bounded size")
...
def _parts(value: object) -> tuple[Mapping[str, Any], str, int, str, str | None]:
    if not isinstance(value, Mapping) or value.get("jsonrpc") != "2.0" or value.get("method") != "event":
        _fail("observed frame is not a Gateway event")
    params = value.get("params")
    if not isinstance(params, Mapping) or not isinstance(params.get("type"), str) or not _SAFE.fullmatch(params["type"]):
        _fail("event type is not bounded")
    kind = params["type"]
    ...
    if terminal is not None and terminal not in {"completed", "denied", "failed", "cancelled"}:
        _fail("event terminal status is unsupported")
    return params, kind, size, digest, terminal
def _root(value: str | Path) -> Path:
    if not isinstance(value, (str, Path)):
        _fail("fixture root is invalid")
    root = Path(value)
    if not root.is_absolute() or ".." in root.parts or root.is_symlink():
        _fail("fixture root is not an existing task-local directory")
    try:
        resolved = root.resolve(strict=True)
    except (OSError, RuntimeError):
        _fail("fixture root is not an existing task-local directory")
    if not resolved.is_dir() or resolved == Path(resolved.anchor) or resolved.is_symlink():
        _fail("fixture root is not an existing task-local directory")
    return resolved
def snapshot_fixture_state(task_root: str | Path, *, max_bytes: int = MAX_FIXTURE_BYTES) -> dict[str, object]:
    if type(max_bytes) is not int or not 1 <= max_bytes <= 1_048_576:
        _fail("fixture state bound is invalid")
    path = _root(task_root) / STATE_FILE
    if path.is_symlink():
        _fail("fixture state is a symlink")
    if not path.exists():
        return {"exists": False, "size": 0, "sha256": None, "record_count": 0}
    if not path.is_file():
        _fail("fixture state is not a regular file")
    try:
...
class V4GatewayObserver:
    ...
    def _session(self, value: object) -> None:
        ...
        if self._session_hash is not None and digest != self._session_hash:
            _fail("session identity does not match")
    ...
    def _event(self, value: object) -> None:
        ...
        if lowered in _START or lowered in _COMPLETE:
            ...
            elif name not in self._starts or self._completes.count(name) >= self._starts.count(name):
        ...
        if any(marker in lowered for marker in _CHILD):
        ...
        if terminal is not None:
            if self._pending_hash is not None or Counter(self._starts) != Counter(self._completes) or self._task_count is not None and len(self._child_complete) != self._task_count:
