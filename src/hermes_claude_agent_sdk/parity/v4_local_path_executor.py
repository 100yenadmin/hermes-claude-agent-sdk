...
_HOST = Path("/Users/m1/repos/hermes-agent-runtime-plugin-api")
_MAP_SHA256 = "85583a44b797a58e6a3f6fcc9f4f5234b445b49c5ab6bf38b153e872473a16ff"
_STATE = ".v4_local_runtime_fixture_state.json"
_BASIC = "openclaw_active/source-docs-discovery-report"
_STATE_ROW = "v2_non_soak/PARENT-01"
_TOOL = "v2_non_soak/TOOL-02"
_APPROVAL = "clawprobench_native/constraints_23_external_approval_boundary_live"
_ROWS = {_BASIC: "basic", _STATE_ROW: "state", _TOOL: "tool", _APPROVAL: "approval"}
_EVENTS = {"basic": ("start", "terminal"), "state": ("start", "state", "terminal"), "tool": ("start", "tool_requested", "tool_result", "state", "terminal"), "approval": ("start", "approval_requested", "approval_decision", "terminal")}
...
def _run_attempt(plugin: Any, registration: Any, dispatch: Any, number: int) -> dict[str, Any]:
    class Agent:
        ...
        def _execute_tool_calls(self, assistant_message: Any, turn_messages: list[dict[str, Any]], task_id: str) -> None:
            ...
            result = handle_function_call(call.function.name, args, task_id=task_id, session_id=self.session_id, tool_call_id=call.id, turn_id=task_id, skip_tool_request_middleware=True, skip_tool_execution_middleware=True)
    ...
    return {"terminal": terminal, "completed": outcome.completed, "execute_calls": host.execute_calls, "result_ok": host.result_ok, "events": [type(event).__name__ for event in outcome.events]}
...
def execute_v4_local_path(*, row_key: str, trial_index: int, path: str, task_root: str | Path) -> dict[str, Any]:
    ...
    if not _HOST.is_dir() or not (_HOST / "agent" / "runtime_api.py").is_file():
        _fail("fixed host runtime is unavailable")
    host_str = str(_HOST)
    ...
    if Path(getattr(dispatch, "__file__", "")).resolve() != _HOST / "agent" / "runtime_dispatch.py" or Path(getattr(host_api, "__file__", "")).resolve() != _HOST / "agent" / "runtime_api.py":
